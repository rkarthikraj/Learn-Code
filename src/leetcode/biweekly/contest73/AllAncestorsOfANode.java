...
class AllAncestorsOfANode {
    ...
    public List<List<Integer>> getAncestors(int n, int[][] edges) {
        ...
        for (int i = 0; i < edges.length; ++i) {
            List<Integer> values = hm.get(edges[i][1]);
            if (values == null) {
                values = new ArrayList<>();
                hm.put(edges[i][1], values);
            }
            if (!values.contains(edges[i][0])) {
                values.add(edges[i][0]);
            }
        }
        ...
        for (int i = 0; i < n; ++i) {
            ...
            List<Integer> values = hm.get(i);
            if (values != null) {
                innerList = ancestors(innerList, hm, values);
            }
    ...
    public List<Integer> ancestors(List<Integer> innerList, Map<Integer, List<Integer>> hm, List<Integer> values) {
        if (values != null) {
            for (int i = 0; i < values.size(); ++i) {
                int value = values.get(i);
                if (!innerList.contains(value)) {
                    innerList.add(value);
                    List<Integer> nestedValues = hm.get(value);
                    ancestors(innerList, hm, nestedValues);
                }
            }
        }
        return innerList;
