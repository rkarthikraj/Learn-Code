...
class GenerateParantheses {
    ...
    public List<String> generateParenthesis(int n) {
        ...
        possiblities(n, n, "", result);
        System.out.println(result);
        return result;
    }

    public void possiblities(int left, int right, String parenthesis, List<String> result) {
        if (left == 0 && right == 0) {
        ...
        if (left > 0) {
            possiblities(left - 1, right, parenthesis + "(", result);
        }

        if (left < right) {
            possiblities(left, right - 1, parenthesis + ")", result);
        }
    }
}
...
class Solution {
    public List<String> generateParentheses1(int numPairs) {
        List<String> result = new ArrayList<>();
